...
exports.encrypt = async (text) => {
  try {
    if (!text || typeof text !== "string") {
      throw new Error("암호화할 대상의 타입이 string이 아님");
    }
    ...
    throw customErrorResponse({ status: 500, message: err.message || "암호화 중 오류 발생" });
  }
};
...
exports.decrypt = async (encryptedText) => {
  try {
    if (!encryptedText || typeof encryptedText !== "string") {
      throw new Error("복호화할 대상의 타입이 string이 아님");
    }
    ...
    throw customErrorResponse({ status: 500, message: err.message || "복호화 중 오류 발생" });
  }
};
