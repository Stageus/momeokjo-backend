...
describe("POST /signin", () => {
  ...
  it("로그인 성공한 경우 상태코드 200을 응답해야한다.", async () => {
    ...
    const cookies = res.headers["set-cookie"];
    expect(cookies).toBeDefined();
    const hasAccessTokenCookie = cookies.some((cookie) => cookie.startsWith("accessToken="));
    expect(hasAccessTokenCookie).toBe(true);

    const hasRefreshTokenCookie = cookies.some((cookie) => cookie.startsWith("refreshToken="));
